...
import { ChanceCardInterface, PlayerEventsCallback, PlayerInterface, PropertyInterface } from "../interfaces";
import { Buff, PlayerInfo, UserInRoomInfo } from "../types";
...
export class Player implements PlayerInterface {
	...
	public loseCard = async (cardId: string) => {
		...
		if (!card) return;
		const targetCardId = card.getId();
		const index = this.chanceCards.findIndex((_card) => _card.getId() === targetCardId);
		if (index != -1) {
	...
	public updateBuff(buffId: string, newBuff: Buff) {
		Array.from(this.callBackMap.values()).forEach((item) => {
			item.forEach((obj) => {
